...
import { useCoinChart } from "@/hooks/use-coin-detail"
...
export function CoinChart({ coinId, name, symbol, price, change24h }: CoinChartProps) {
  const [activeRange, setActiveRange] = useState<Range>("30D")
  const days = activeRange === "7D" ? 7 : 30
  const isPositive = change24h >= 0

  const { data: priceHistory, isPending, isError } = useCoinChart(coinId, days)
